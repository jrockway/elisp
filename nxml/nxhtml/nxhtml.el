...
;; Created: 2005-08-05
(defconst nxhtml:version "1.17") ;;Version:
;; Last-Updated: 2008-03-14T02:19:33+0100 Fri
;; Keywords: languages
;; Features that might be required by this library:
;;
;;   `appmenu', `appmenu-fold', `as-external', `button', `cl',
;;   `compile', `cus-edit', `cus-face', `cus-load', `cus-start',
;;   `easymenu', `flymake', `flymake-js', `flymake-php', `fold-dwim',
;;   `fupd', `gimp', `help-mode', `hexcolor', `hideshow',
;;   `html-chklnk', `html-imenu', `html-move', `html-pagetoc',
;;   `html-quote', `html-site', `html-toc', `html-upl', `html-wtoc',
;;   `loadhist', `mail-prsvr', `mlinks', `mm-util', `mumamo',
;;   `nxhtml-menu', `nxhtml-mumamo', `nxhtml-strval', `nxml-enc',
;;   `nxml-glyph', `nxml-mode', `nxml-ns', `nxml-outln',
;;   `nxml-parse', `nxml-rap', `nxml-util', `nxml-where',
;;   `ourcomments-util', `outline', `popcmp', `rng-dt', `rng-loc',
;;   `rng-match', `rng-parse', `rng-pttrn', `rng-uri', `rng-util',
;;   `rng-valid', `rngalt', `server', `setup-helper', `sgml-mode',
;;   `tidy-xhtml', `timer', `tool-bar', `typesetter', `url-expand',
;;   `url-methods', `url-parse', `url-util', `url-vars', `view',
;;   `w32-reg-iface', `w32-regdat', `w32shell', `wid-edit',
;;   `xhtml-help', `xml', `xmltok'.
;;
...
(defun nxhtml-browse-file ()
  ...
  (interactive)
  (let ((use-temp (or nxhtml-current-validation-header
                      (buffer-modified-p)
...
(defun nxhtml-complete-first-try ()
  (when (and
         ...
         (if (= 0 (buffer-size))
           ...
           (save-restriction
             (widen)
             (looking-at (rx buffer-start (0+ space) buffer-end))))
         (not nxhtml-validation-header-mode))
    (nxhtml-empty-page-completion)))
