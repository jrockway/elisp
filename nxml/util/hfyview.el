...
;; Version: 0.62
;; Last-Updated: Fri Dec 28 03:04:22 2007 (3600 +0100)
;; Keywords: printing
...
;;  `hfyview-add-to-files-menu' to t. This will add an entry "Quick
;;  Print (to OS Default Printer) on the files menu.
;;
;;
...
(defcustom hfyview-add-to-files-menu t
...
(defun hfyview-fontify-win-to (win tag whole-buffer)
  (let* ((bstart (unless whole-buffer (window-start win)))
    ...
    (with-current-buffer hbuf
      ...
      (setq start (point))
      (insert
       (format "<%s style=\"width:%sem; height:%sem; border: 1px solid %s; overflow:auto; padding:4px;\">\n"
